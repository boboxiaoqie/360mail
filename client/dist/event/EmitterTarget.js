...
export default class EmitterTarget {
    ...
    addEventListener(type, handler) {
        if (!this.list[type])
            this.list[type] = new Set();
        this.list[type].add(handler);
    }
    removeEventListener(type, handler) {
        if (!this.list[type])
            return;
        if (this.list[type].has(handler))
            this.list[type].delete(handler);
    }
    dispatchEvent(evt) {
        if (!this.list[evt.type])
            return;
        evt.target = this;
        this.list[evt.type].forEach((item) => item.call(this, evt));
    }
}
